...
The encoder outputs the mean $\mu$ and standard deviation $\sigma$ of the latent distribution, and we sample a latent vector $z\sim\mathcal{N}(\mu, diag(\sigma^2))$ from this distribution using:
...
1. Reconstruction loss: $L_{reconstruction}=||g(f(x))-x||^2_2$. This measures how well the decoder reconstructs the input from the latent vector.
2. KL divergence: $L_{variational}=dist(\mathcal{N}(0, I), \mathcal{N}(\mu, diag(\sigma^2)))$. This measures how close the learned latent distribution is to the normal distribution.
