...
## Modivation
<p>
Given example sentence: the cow jumped over the moon. 
Covert the sentence into an nxd tensor (n is the number of words, d is the vector length) where each words is one-hot encoded into a 
</p>
...
<center><img src="images/selfAttentionDiagram1.pdf" width="700"></center>
...
\begin{center}
    $y^{(i)}=\sum_{i=1}^n W^{(v)}x^{i}soft(<W^{(k)}x^{(i)},W^{(q)}x^{(j)}>)$ \\
    $\Rightarrow W^{(v)}\sum_{i=1}^n x^{i}soft(x^{(i)^T}W^{(k)^T}W^{(q)}x^{(j)})$
\end{center}
...
\begin{center}    
   $W^{(v)}\sum_{i=1}^n x^{i}soft(x^{(i)^T}W^{(k)^T}W^{(q)}x^{(j)})$\\
   $\Rightarrow soft(XW^{(q)^T}W^{(k)^T}X)XW^{(v)}$
\end{center}
...
2. Self-attention layers are stacked on top of each other 
	a. Some models use as much as 96 layers in sequence.  Also possible to utilize them with multiple self-attentions in one layer.
...
<center><img src="images/crossAttentionDiagram1.pdf" width="700"></center>
</p>
## Positional Encoding
<p>
